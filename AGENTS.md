...
- **Instalação Global e Elevação de Privilégios (`elevation.rs`)**: Adicionado suporte para instalação em `/opt/appimages` e atalhos em `/usr/share/applications` via flag `--global`. O sistema agora solicita elevação via `sudo` (CLI) ou `pkexec` (TUI) de forma transparente.
- **Segurança e Validação de Inputs (`security.rs`)**: Implementado módulo de segurança para sanitizar nomes de aplicativos e validar caminhos de instalação, prevenindo injeção de comandos e acesso não autorizado a arquivos do sistema durante operações com privilégios elevados.
- **Identificação Visual na TUI (`tui_render.rs`)**: Aplicativos instalados globalmente agora recebem uma marcação visual `[Global]` na cor ciano para fácil identificação pelo usuário.
